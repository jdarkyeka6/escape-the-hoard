/* =========================================================================
   ESCAPE THE HORDE — co-op networking, PHASE 2: SHARED ZOMBIES
   
   This replaces net.js and adds:
   - Host-authoritative zombie simulation
   - Delta compression (only send changed zombies)
   - Keyframe sync every second for recovery
   - Damage claiming system (claim hits, wait for authority)
   - Host migration (lowest ID becomes new host if current host drops)
   ========================================================================= */
'use strict';
window.NET = (function () {

const H = window.HORDE;
const T = window.THREE;
if (!H || !T) { console.warn('[net] game not loaded'); return {}; }

const CFG = window.HORDE_CONFIG || {};
const SUPABASE_URL = CFG.supabaseUrl || 'https://YOUR-PROJECT.supabase.co';
const SUPABASE_ANON_KEY = CFG.supabaseAnonKey || 'YOUR-ANON-KEY';
const configured = SUPABASE_URL.indexOf('YOUR-PROJECT') === -1;

const SEND_HZ    = 18;        // player position updates per second
const ZOMBIE_HZ  = 12;        // zombie state updates per second
const INTERP_MS  = 120;       // interpolation delay
const DROP_MS    = 6000;      // silence timeout
const KEYFRAME_INTERVAL = 1000; // full zombie sync every 1 second

const state = {
  on: false, room: null, id: null, name: null,
  client: null, channel: null,
  peers: new Map(),           // id -> peer record
  sendT: 0, seq: 0,
  isHost: false, started: false,
  onRoster: null,
  
  // Phase 2: Shared zombie data
  zombieMap: new Map(),       // zombieIndex -> {prev, next, lastSeen}
  zombieDelta: [],            // pending zombie updates to send
  keyframeT: 0,               // timer for keyframe broadcasts
  pendingHits: [],            // damage claimed but not applied yet
};

/* ================================================================ avatar */
const bodyGeo = new T.BoxGeometry(1, 1, 1);
const AV_COLS = [0x4ea8de, 0xe8b04b, 0x6fd66f, 0xd96f9a, 0xb07fe0, 0x59d6c8];

function makeAvatar (name, colIdx) {
  const col = AV_COLS[colIdx % AV_COLS.length];
  const g = new T.Group();
  const kit  = new T.MeshLambertMaterial({ color: col });
  const dark = new T.MeshLambertMaterial({ color: 0x2b2f38 });
  const skin = new T.MeshLambertMaterial({ color: 0xb98a63 });

  const put = (mat, x, y, z, sx, sy, sz) => {
    const m = new T.Mesh(bodyGeo, mat);
    m.position.set(x, y, z);
    m.scale.set(sx, sy, sz);
    m.castShadow = true; m.receiveShadow = true;
    g.add(m);
    return m;
  };

  const torso = put(kit,  0, 1.15, 0, 0.52, 0.72, 0.30);
  const head  = put(skin, 0, 1.68, 0, 0.28, 0.30, 0.28);
  put(dark, 0, 1.80, 0, 0.31, 0.10, 0.31);
  const armL = put(kit, -0.36, 1.22, 0, 0.16, 0.62, 0.16);
  const armR = put(kit,  0.36, 1.22, 0, 0.16, 0.62, 0.16);
  const legL = put(dark, -0.15, 0.42, 0, 0.19, 0.84, 0.19);
  const legR = put(dark,  0.15, 0.42, 0, 0.19, 0.84, 0.19);

  const beacon = H.glowSprite ? H.glowSprite(col, 2.4, 0.45) : null;
  if (beacon) { beacon.position.y = 1.95; g.add(beacon); }

  g.add(makeNameTag(name, col));
  H.scene.add(g);
  return { g, torso, head, armL, armR, legL, legR, col };
}

function makeNameTag (name, col) {
  const c = document.createElement('canvas');
  c.width = 256; c.height = 64;
  const x = c.getContext('2d');
  x.font = 'bold 34px system-ui, sans-serif';
  x.textAlign = 'center';
  x.textBaseline = 'middle';
  x.lineWidth = 6;
  x.strokeStyle = 'rgba(0,0,0,.85)';
  x.strokeText(name, 128, 34);
  x.fillStyle = '#' + col.toString(16).padStart(6, '0');
  x.fillText(name, 128, 34);
  const tex = new T.CanvasTexture(c);
  const s = new T.Sprite(new T.SpriteMaterial({ map: tex, transparent: true,
                                                depthTest: false, toneMapped: false }));
  s.scale.set(2.0, 0.5, 1);
  s.position.y = 2.35;
  s.renderOrder = 50;
  return s;
}

/* ================================================================= peers */
function addPeer (id, name, host) {
  if (state.peers.has(id) || id === state.id) return;
  const av = makeAvatar(name || 'SURVIVOR', state.peers.size + 1);
  state.peers.set(id, {
    id, name, av, host: !!host,
    prev: null, next: null,
    lastSeen: performance.now(),
    walk: 0,
  });
  H.toast(name + ' JOINED');
}

function dropPeer (id) {
  const p = state.peers.get(id);
  if (!p) return;
  H.scene.remove(p.av.g);
  state.peers.delete(id);
  H.toast((p.name || 'A SURVIVOR') + ' LEFT');
  
  // If host left, promote lowest remaining id
  if (state.isHost) return; // we stay host
  checkHostMigration();
}

function checkHostMigration () {
  if (state.isHost || !state.on) return;
  const allIds = [state.id, ...state.peers.keys()].sort();
  const lowest = allIds[0];
  if (lowest === state.id) {
    state.isHost = true;
    H.toast('YOU ARE NOW HOST');
  }
}

/* ============================================================= player sync */
function localSnapshot () {
  const pl = H.player;
  return {
    i: state.id,
    n: ++state.seq,
    t: Date.now(),
    x: +pl.pos.x.toFixed(2),
    y: +(pl.pos.y - H.EYE).toFixed(2),
    z: +pl.pos.z.toFixed(2),
    r: +pl.yaw.toFixed(2),
    p: +pl.pitch.toFixed(2),
    s: Math.round(Math.hypot(pl.vel.x, pl.vel.z)),
    c: pl.crouch > 0.5 ? 1 : 0,
    h: Math.round(pl.hp),
    w: H.gunState.cur,
  };
}

function onSnapshot (m) {
  if (!m || m.i === state.id) return;
  let p = state.peers.get(m.i);
  if (!p) { addPeer(m.i, m.nm || 'SURVIVOR'); p = state.peers.get(m.i); }
  if (!p) return;
  if (p.next && m.n <= p.next.n) return;
  p.prev = p.next || m;
  p.next = m;
  p.lastSeen = performance.now();
}

function onRemoteShot (m) {
  if (!m || m.i === state.id) return;
  const from = new T.Vector3(m.ax, m.ay, m.az);
  const to   = new T.Vector3(m.bx, m.by, m.bz);
  H.tracer(from, to);
  if (H.Sfx && H.Sfx.shot) {
    const w = H.WEAPONS[m.w] || H.WEAPONS[0];
    H.Sfx.shot(w.tone);
  }
}

/* ============================================================== zombie sync */
/* Only host runs zombie AI. Guests receive snapshots and interpolate.
   
   Zombie snapshot format: { idx, x, z, y (feet), r (yaw), s (state), t (type), a (alive), hp }
   
   Host broadcasts:
   - Delta updates 12x/sec: only changed zombies
   - Keyframes 1x/sec: all zombies (resync guard)
   
   Guest damage claiming:
   - "I hit zombie 5 for 40 dmg (headshot)"
   - Wait for host to apply and send back in next snapshot
*/

function onZombieDelta (msg) {
  if (!msg || !msg.zombies) return;
  
  msg.zombies.forEach(snap => {
    let entry = state.zombieMap.get(snap.idx);
    if (!entry) {
      entry = { prev: null, next: null, lastSeen: performance.now() };
      state.zombieMap.set(snap.idx, entry);
    }
    entry.prev = entry.next || snap;
    entry.next = snap;
    entry.lastSeen = performance.now();
  });
}

function onZombieKeyframe (msg) {
  if (!msg || !msg.zombies) return;
  state.zombieMap.clear();
  msg.zombies.forEach(snap => {
    state.zombieMap.set(snap.idx, {
      prev: null, next: snap, lastSeen: performance.now()
    });
  });
}

function onDamageAck (msg) {
  if (!msg || msg.i === state.id) return;
  // Remove the acknowledged damage from pending hits
  state.pendingHits = state.pendingHits.filter(h => !(
    h.idx === msg.idx && h.dmg === msg.dmg && h.seq === msg.seq
  ));
}

/* Build a snapshot of all living zombies. Only called by host. */
function buildZombieSnapshot () {
  const zombies = [];
  if (!H.zombies) return zombies;
  
  H.zombies.forEach((z, idx) => {
    if (!z || !z.alive) return;
    zombies.push({
      idx,
      x: +z.pos.x.toFixed(1),
      z: +z.pos.z.toFixed(1),
      y: +z.pos.y.toFixed(1),
      r: +z.yaw.toFixed(2),
      s: z.state,     // attack, walk, stagger, etc.
      t: z.type,      // 'green', 'blue', 'yellow', etc.
      a: 1,           // alive
      hp: Math.round(z.hp),
    });
  });
  
  return zombies;
}

/* Compare two snapshots and return only what changed. */
function computeZombieDelta (prev, curr) {
  if (!prev) return curr;
  const delta = [];
  const prevMap = new Map(prev.map(z => [z.idx, z]));
  
  curr.forEach(z => {
    const p = prevMap.get(z.idx);
    if (!p) {
      delta.push(z); // new zombie
      return;
    }
    // Check if any field changed
    if (p.x !== z.x || p.z !== z.z || p.y !== z.y || p.r !== z.r || 
        p.s !== z.s || p.a !== z.a || p.hp !== z.hp) {
      delta.push(z);
    }
  });
  
  // Dead zombies
  prevMap.forEach((p, idx) => {
    if (!curr.some(z => z.idx === idx)) {
      delta.push({ idx, a: 0 }); // zombie died
    }
  });
  
  return delta;
}

/* Render remote zombie at interpolated position. Called once per frame per zombie. */
function updateRemoteZombie (entry, now) {
  if (!entry.next || !H.zombieById) return;
  
  const a = entry.prev || entry.next;
  const b = entry.next;
  const span = Math.max(1, (b.t || Date.now()) - (a.t || Date.now()));
  const k = Math.max(0, Math.min(1, (now - (a.t || Date.now())) / span));
  
  const x = a.x + (b.x - a.x) * k;
  const z = a.z + (b.z - a.z) * k;
  const y = a.y + (b.y - a.y) * k;
  const r = a.r + ((b.r - a.r + Math.PI) % (Math.PI * 2) - Math.PI) * k;
  
  const z_obj = H.zombieById(entry.next.idx);
  if (!z_obj) return;
  
  z_obj.pos.set(x, y, z);
  z_obj.yaw = r;
  z_obj.hp = entry.next.hp;
  z_obj.alive = entry.next.a ? true : false;
}

/* ============================================================== per frame */
function update (dt) {
  if (!state.on || !state.channel) return;

  // OUTBOUND: player position
  state.sendT -= dt;
  if (state.sendT <= 0) {
    state.sendT = 1 / SEND_HZ;
    const snap = localSnapshot();
    snap.nm = state.name;
    state.channel.send({ type: 'broadcast', event: 'pos', payload: snap });
  }

  // OUTBOUND: host broadcasts zombies
  if (state.isHost && state.started) {
    state.keyframeT -= dt;
    const curr = buildZombieSnapshot();
    
    if (state.keyframeT <= 0) {
      // Full keyframe
      state.keyframeT = KEYFRAME_INTERVAL / 1000;
      state.channel.send({
        type: 'broadcast',
        event: 'z_keyframe',
        payload: { t: Date.now(), zombies: curr }
      });
      state.zombieDelta = curr;
    } else {
      // Delta updates
      const delta = computeZombieDelta(state.zombieDelta, curr);
      if (delta.length > 0) {
        state.channel.send({
          type: 'broadcast',
          event: 'z_delta',
          payload: { t: Date.now(), zombies: delta }
        });
      }
      state.zombieDelta = curr;
    }
  }

  // INBOUND: render remote players
  const now = Date.now() - INTERP_MS;
  const clock = performance.now();

  state.peers.forEach((p, id) => {
    if (clock - p.lastSeen > DROP_MS) { dropPeer(id); return; }
    if (!p.next) return;
    
    const a = p.prev || p.next, b = p.next;
    const span = Math.max(1, b.t - a.t);
    const k = Math.max(0, Math.min(1, (now - a.t) / span));

    const x = a.x + (b.x - a.x) * k;
    const z = a.z + (b.z - a.z) * k;
    const y = a.y + (b.y - a.y) * k;
    
    let dr = b.r - a.r;
    while (dr >  Math.PI) dr -= Math.PI * 2;
    while (dr < -Math.PI) dr += Math.PI * 2;
    const r = a.r + dr * k;

    const av = p.av;
    av.g.position.set(x, y, z);
    av.g.rotation.y = r;

    const crouch = b.c ? 1 : 0;
    av.g.scale.y = 1 - crouch * 0.32;

    p.walk += dt * b.s * 1.4;
    const sw = Math.sin(p.walk) * Math.min(1, b.s / 6);
    av.legL.rotation.x =  sw * 0.7;
    av.legR.rotation.x = -sw * 0.7;
    av.armL.rotation.x = -sw * 0.4;
    av.armR.rotation.x =  sw * 0.4;
    av.head.rotation.x = -(b.p || 0) * 0.6;
  });

  // INBOUND: render remote zombies
  const nowZ = Date.now() - INTERP_MS;
  state.zombieMap.forEach((entry, idx) => {
    if (clock - entry.lastSeen > DROP_MS) {
      state.zombieMap.delete(idx);
      return;
    }
    updateRemoteZombie(entry, nowZ);
  });

  // Host applies pending damage claims
  if (state.isHost) {
    state.pendingHits.forEach(claim => {
      const z = H.zombieById ? H.zombieById(claim.idx) : null;
      if (z && z.alive) {
        H.hurtZombie(claim.idx, claim.dmg, claim.head);
        // Send acknowledgement
        state.channel.send({
          type: 'broadcast',
          event: 'dmg_ack',
          payload: { i: claim.playerId, idx: claim.idx, dmg: claim.dmg, seq: claim.seq }
        });
      }
    });
    state.pendingHits = [];
  }
}

/* ================================================================ joining */
function makeRoomCode () {
  const L = 'BCDFGHJKLMNPQRSTVWXZ';
  let s = '';
  for (let i = 0; i < 4; i++) s += L[(Math.random() * L.length) | 0];
  return s;
}

async function host (name) {
  return join(makeRoomCode(), name, true);
}

let sbLoading = null;

function loadSupabase () {
  if (window.supabase) return Promise.resolve(true);
  if (sbLoading) return sbLoading;
  sbLoading = new Promise(resolve => {
    let settled = false;
    const done = ok => { if (!settled) { settled = true; resolve(ok && !!window.supabase); } };
    const el = document.createElement('script');
    el.src = 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';
    el.onload  = () => done(true);
    el.onerror = () => done(false);
    document.head.appendChild(el);
    setTimeout(() => done(false), 8000);
  });
  return sbLoading;
}

async function join (room, name, asHost) {
  if (state.on) return;
  if (!configured) { H.toast('SET YOUR KEYS IN config.js'); return; }
  H.toast('CONNECTING…');
  const ready = await loadSupabase();
  if (!ready) {
    sbLoading = null;
    H.toast('CANNOT REACH SUPABASE — CHECK NETWORK');
    pushRoster();
    return;
  }

  state.name = (name || 'SURVIVOR').toUpperCase().slice(0, 12);
  state.room = (room || 'STREET').toUpperCase().slice(0, 12);
  state.id = Math.random().toString(36).slice(2, 10);
  state.isHost = !!asHost;
  state.started = false;

  state.client = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    realtime: { params: { eventsPerSecond: 30 } },
  });

  state.channel = state.client.channel('horde:' + state.room, {
    config: { presence: { key: state.id }, broadcast: { self: false } },
  });

  state.channel
    .on('broadcast', { event: 'pos'      }, ({ payload }) => onSnapshot(payload))
    .on('broadcast', { event: 'shot'     }, ({ payload }) => onRemoteShot(payload))
    .on('broadcast', { event: 'start'    }, ({ payload }) => onStart(payload))
    .on('broadcast', { event: 'z_delta'  }, ({ payload }) => onZombieDelta(payload))
    .on('broadcast', { event: 'z_keyframe' }, ({ payload }) => onZombieKeyframe(payload))
    .on('broadcast', { event: 'dmg_ack'  }, ({ payload }) => onDamageAck(payload))
    .on('presence',  { event: 'join'     }, ({ key, newPresences }) => {
      if (key === state.id) return;
      const n = newPresences && newPresences[0];
      addPeer(key, (n && n.name) || 'SURVIVOR', n && n.host);
      pushRoster();
    })
    .on('presence',  { event: 'sync'     }, () => {
      const all = state.channel.presenceState();
      Object.keys(all).forEach(key => {
        if (key === state.id) return;
        const n = all[key] && all[key][0];
        addPeer(key, (n && n.name) || 'SURVIVOR', n && n.host);
      });
      pushRoster();
    })
    .on('presence',  { event: 'leave'    }, ({ key }) => { dropPeer(key); pushRoster(); })
    .subscribe(async (status) => {
      if (status !== 'SUBSCRIBED') return;
      await state.channel.track({ name: state.name, host: state.isHost, at: Date.now() });
      state.on = true;
      H.toast(state.isHost ? 'HOSTING ROOM ' + state.room : 'JOINED ROOM ' + state.room);
      pushRoster();
    });
}

function leave () {
  if (!state.on) return;
  state.peers.forEach((p, id) => dropPeer(id));
  if (state.channel) state.channel.unsubscribe();
  state.on = false; state.isHost = false; state.started = false;
  state.zombieMap.clear();
  pushRoster();
  H.toast('LEFT THE ROOM');
}

function roster () {
  const list = [{ id: state.id, name: state.name, host: state.isHost, me: true }];
  state.peers.forEach(p => list.push({ id: p.id, name: p.name, host: !!p.host, me: false }));
  return list;
}

function pushRoster () {
  if (state.onRoster) state.onRoster(roster(), state);
}

function sendStart () {
  if (!state.on || !state.isHost || !state.channel) return;
  state.started = true;
  state.channel.send({ type: 'broadcast', event: 'start', payload: { i: state.id } });
}

function onStart (m) {
  if (!m || m.i === state.id || state.started) return;
  state.started = true;
  H.toast('HOST STARTED THE RUN');
  H.restart();
}

function onLocalShot (from, to, weaponIndex) {
  if (!state.on || !state.channel) return;
  state.channel.send({
    type: 'broadcast', event: 'shot',
    payload: { i: state.id, w: weaponIndex,
               ax:+from.x.toFixed(2), ay:+from.y.toFixed(2), az:+from.z.toFixed(2),
               bx:+to.x.toFixed(2),   by:+to.y.toFixed(2),   bz:+to.z.toFixed(2) },
  });
}

/* Claim damage on a zombie. Host will apply it and send back acknowledgement. */
function claimDamage (zombieIndex, dmg, isHeadshot) {
  if (!state.on || !state.channel || state.isHost) {
    // If we're the host, apply immediately
    if (state.isHost && H.hurtZombie) {
      H.hurtZombie(zombieIndex, dmg, isHeadshot);
    }
    return;
  }
  
  const claim = {
    playerId: state.id,
    idx: zombieIndex,
    dmg,
    head: isHeadshot ? 1 : 0,
    seq: state.seq++,
  };
  
  state.pendingHits.push(claim);
  state.channel.send({
    type: 'broadcast',
    event: 'dmg_claim',
    payload: claim
  });
}

function roomFromUrl () {
  try { return new URLSearchParams(location.search).get('room'); }
  catch (e) { return null; }
}

function shareLink () {
  if (!state.room) return location.href;
  return location.origin + location.pathname + '?room=' + encodeURIComponent(state.room);
}

async function copyLink () {
  try {
    await navigator.clipboard.writeText(shareLink());
    H.toast('INVITE LINK COPIED');
  } catch (e) {
    H.toast(shareLink());
  }
}

return { join, host, leave, update, onLocalShot, state, roomFromUrl,
         shareLink, copyLink, configured, roster, sendStart, makeRoomCode,
         claimDamage, checkHostMigration };

})();
